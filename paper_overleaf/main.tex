...
% Gap in literature
Existing benchmarks evaluate CD frameworks on a single model size \citep{garg2025jsonschemabench}, leaving open the question of how CD interacts with model scale.
This question is practically important: if CD can rescue a 1B model, practitioners may not need a 4B model.

% Research question
We ask: \textbf{Does \cd{} flatten the small-model scaling curve for structured output?}
...
% STED: temperature × consistency, no CD
% Renze & Guven: temperature × accuracy, no structured output
\citet{garg2025jsonschemabench} introduce JSONSchemaBench, evaluating six CD frameworks on structured output tasks.
...
Other work studies the effect of temperature on output consistency \citep{sted2024, renze2024temperature} but does not consider CD or structured output specifically.
...
\begin{tabular}{lccc}
\toprule
\textbf{Work} & \textbf{CD?} & \textbf{Size ladder?} & \textbf{Focus} \\
\midrule
JSONSchemaBench & \checkmark & $\times$ & Framework efficiency \\
STED & $\times$ & $\times$ & Temperature $\times$ consistency \\
Renze \& Guven & $\times$ & $\times$ & Temperature $\times$ accuracy \\
\textbf{This work} & \checkmark & \checkmark & CD $\times$ scale interaction \\
\bottomrule
\end{tabular}
...
We select five models representing three families and a controlled size range (0.6B--4B).
...
All conditions use greedy decoding ($T=0$, \texttt{do\_sample=False}) for determinism and reproducibility.
...
\emph{Schema validity}: does the parsed output validate against the task's JSON schema (Draft 2020-12)?
...
\begin{tabular}{lll}
\toprule
\textbf{Archetype} & \textbf{Example} & \textbf{CD prediction} \\
\midrule
Type coercion & \texttt{"4.98"} vs \texttt{4.98} & Rescuable \\
Structural misplacement & Missing required field & Rescuable \\
Instruction-semantic & Missing function call & Resistant \\
\bottomrule
\end{tabular}
